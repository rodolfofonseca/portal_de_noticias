<?php
require_once 'controller/modelos/Estado.php';
require_once 'controller/utilidades/DAO.php';
require_once 'controller/utilidades/LogDoSistema.php';
class EstadoCtr
{
    private $dao;
    private $logNoSistema;
    public function __construct()
    {
        $this->dao = new DAO();
        $this->logNoSistema = new LogDoSistema();
    }
    public function Salvar($model)
    {
        try {
            $dados = array($model->getPais()->getIdPais(), $model->getNomeEstado(), $model->getSigla());
            return $this->dao->Salvar('estado', 'id_pais, nome_estado, sigla', $dados);
        } catch (Exception $ex) {
            $this->logNoSistema->EscreverArquivo('logNoSistema.txt', $ex->getMessage());
            return false;
        }
    }
    public function Alterar($model)
    {
        try {
            $dados = array($model->getPais()->getIdPais(), $model->getNomeEstado(), $model->getSigla(), $model->getIdEstado());
            $campos = array('id_pais', 'nome_estado', 'sigla');
            return $this->dao->Alterar('estado', $campos, $dados, 'id_estado');
        } catch (Exception $ex) {
            $this->logNoSistema->EscreverArquivo('logNoSistema.txt', $ex->getMessage());
            return false;
        }
    }
    public function Pesquisar($comando){
        try{
            return $this->dao->Pesquisar($comando);
        }catch(Exception $ex){
            $this->logDoSistema->EscreverArquivo('logDoSistema', $ex->getMessage());
            return array();
        }
    }
}
