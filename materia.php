    ...
    foreach ($retorno_noticias as $noticia) {
        ...
        if (empty($menu_retorno) == false) {
            foreach ($menu_retorno as $menu) {
                $descricao_menu = (string) $menu['descricao_menu'];
            }
        }
?>
        <div class="container-fluid">
            <div class="container">
                <nav class="breadcrumb bg-transparent m-0 p-0">
                    ...
                    <span class="breadcrumb-item active"><?php echo $noticia['titulo_noticias']; ?></span>
                </nav>
            </div>
        </div>
                        ...
                        <div class="position-relative mb-3">
                            ...
                            <div class="overlay position-relative bg-light">
                                <div class="mb-3">
                                    ...
                                    <span><?php $dataSistema->RetornarDataCompleta($noticia['data_postagem']);     //Quinta, 10 de Fevereiro de 2022 
                                            ?></span>
                                </div>
                                <div>
                                    <h3 class="mb-3"><?php echo $noticia['titulo_noticias']; ?></h3>
                                    <?php
                                    $paragrafos_retorno = $materiaCtr->Pesquisar("select * from paragrafos where id_noticia = '" . $noticia['id_noticias'] . "';");
                                    if (empty($paragrafos_retorno) == false) {
                                        foreach ($paragrafos_retorno as $paragrafos) {
                                            $contadorParagrafo++;
                                            if($contadorParagrafo == 1 || $contadorParagrafo == 0){
                                                ?>
                                                <div class="mb-3 pb-3">
